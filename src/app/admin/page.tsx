...
export default function AdminPage() {
  ...
  const clearAllData = () => {
    if (
      confirm(
        "정말로 모든 참가자 데이터를 삭제하시겠습니까? 이 작업은 되돌릴 수 없습니다."
      )
    ) {
  ...
  const exportData = () => {
    const participants = JSON.parse(
      localStorage.getItem("year-end-party-participants") || "{}"
    );
    ...
    <div className="min-h-screen bg-gradient-to-br from-gray-50 via-white to-blue-50">
        ...
        <div className="max-w-6xl mx-auto px-4 py-6">
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="text-gray-600">송년회 조 배정 시스템 관리</p>
            </div>
            <Link
              href="/"
      ...
      <main className="max-w-6xl mx-auto px-4 py-8 space-y-8">
        ...
        <div className="bg-white rounded-lg shadow-lg p-6">
          ...
          <div className="grid grid-cols-1 md:grid-cols-4 gap-4 mb-6">
            <div className="bg-blue-50 rounded-lg p-4 text-center">
              ...
              <div className="text-3xl font-bold text-blue-800">
                {YEAR_END_PARTY_CONFIG.TEAM_COUNT}
              </div>
          ...
          <p className="text-center text-sm text-gray-600 mt-2">
            {((stats.totalAssigned / stats.totalCapacity) * 100).toFixed(1)}%
            완료
          </p>
        </div>
        ...
        <div className="bg-white rounded-lg shadow-lg p-6">
          ...
          <div className="mt-6 p-4 bg-gray-50 rounded-lg">
            ...
            <ul className="text-xs text-gray-600 space-y-1">
              <li>
                • <strong>테스트 참가자 생성</strong>: 임의의 참가자를 생성하여
                시스템을 테스트할 수 있습니다
              </li>
              <li>
                • <strong>통계 새로고침</strong>: 최신 참가자 현황을 다시
                로드합니다
              </li>
              <li>
                • <strong>데이터 내보내기</strong>: 모든 참가자 데이터를 JSON
                파일로 다운로드합니다
              </li>
              <li>
                • <strong>모든 데이터 삭제</strong>: localStorage의 모든 참가자
                데이터를 삭제합니다 (주의!)
              </li>
            </ul>
          </div>
              ...
              <div className="space-y-2 text-sm">
                <div className="flex justify-between">
                  ...
                  <span className="font-medium">
                    {YEAR_END_PARTY_CONFIG.TEAM_COUNT}개
                  </span>
                </div>
                <div className="flex justify-between">
                  <span className="text-gray-600">조별 최대 인원:</span>
                  <span className="font-medium">
                    {YEAR_END_PARTY_CONFIG.MEMBERS_PER_TEAM}명
                  </span>
                </div>
                <div className="flex justify-between">
                  <span className="text-gray-600">최대 추첨 번호:</span>
                  <span className="font-medium">
                    {YEAR_END_PARTY_CONFIG.LOTTERY_NUMBER_RANGE.max}번
                  </span>
                </div>
                <div className="flex justify-between">
      ...
      <footer className="bg-gray-50 py-8 border-t">
        <div className="max-w-6xl mx-auto px-4 text-center text-gray-600">
          <p className="text-sm">송년회 2025 - 관리자 페이지</p>
        </div>
      </footer>
    </div>
  );
